...
void run_syscall(uint32_t syscall_number, uint32_t n_args, ...){
    struct syscall_args sargs = {0};
    sargs.number = syscall_number;
    va_list ap;
    va_start(ap, n_args);
...
void sys_breakpoint_handler(arm_context_t* state) {
    ...
    //over-write memory to replace the string the write syscall is going to write
    //lets read and see what is currently in memory
    uint64_t uap = state->ss.ss_64.x[1];
    //get len
    uint32_t clen = rk32(uap+0x10);
    printf("Got length of current string to write: %i\n",clen);
    //create buffer to hold it
    char* buff[clen];
    rkbuffer(uap+8,buff,clen);
    printf("Original string passed to write: %s\n",*buff);
    
    char* replacer_string = strdup("*** Bow WoW ***\n");
    
    printf("Replacing string with: %s",replacer_string);
    
    wk64(uap+8, (uint64_t)replacer_string);
    wk64(uap+0x10, strlen(replacer_string));
    printf("Continue execution, should see print below\n");
...
void test_kdbg() {
    ...
    run_syscall_with_breakpoint(ksym(KSYMBOL_WRITE_SYSCALL_ENTRYPOINT),  // breakpoint address
                                sys_breakpoint_handler,            // breakpoint hit handler
                                4,                                       // SYS_write
                                3,                                       // 3 arguments
                                1,                                       // stdout
                                (uint64_t)mp_str,                // "===meow===\n"
                                strlen(mp_str));                 // 13
